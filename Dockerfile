...
RUN mkdir -pv /opt/soft && \
	cd /opt/soft && \
	echo "编译 GO" && \
	wget -nv https://storage.googleapis.com/golang/go1.10.1.linux-amd64.tar.gz && tar -C /usr/local -xzf go1.10.1.linux-amd64.tar.gz  && mkdir -pv /data/go && go version && go version && \
	cd /opt/soft && \
	echo "编译 Cmake" && \
