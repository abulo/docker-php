...
RUN  sed -i 's/archive.ubuntu.com/mirrors.aliyun.com/' /etc/apt/sources.list && \
	...
	apt-get install --no-install-recommends -y -q  libxml2 libxml2-dev build-essential openssl libssl-dev make curl libjpeg-dev libpng-dev libmcrypt-dev libreadline6 libreadline6-dev libmhash-dev libfreetype6-dev libkrb5-dev libc-client2007e libc-client2007e-dev libbz2-dev libxslt1-dev libxslt1.1 libpq-dev libpng12-dev git autoconf automake m4 libmagickcore-dev libmagickwand-dev libcurl4-openssl-dev libltdl-dev libmhash2 libiconv-hook-dev libiconv-hook1 libpcre3-dev libgmp-dev gcc g++ ssh cmake re2c wget cron bzip2 rcconf flex vim bison mawk cpp binutils libncurses5 unzip tar libncurses5-dev libtool libpcre3 libpcrecpp0v5 zlibc libltdl3-dev slapd ldap-utils db5.3-util libldap2-dev libsasl2-dev net-tools libicu-dev libtidy-dev systemtap-sdt-dev libgmp3-dev gettext libexpat1-dev libz-dev libedit-dev libdmalloc-dev libevent-dev libyaml-dev autotools-dev pkg-config zlib1g-dev libcunit1-dev libev-dev libjansson-dev libc-ares-dev libjemalloc-dev cython python3-dev python-setuptools libreadline-dev perl  python3-pip zsh tcpdump strace gdb openbsd-inetd telnetd htop valgrind libprotobuf-dev libleveldb-dev libsnappy-dev libhdf5-serial-dev protobuf-compiler libatlas-base-dev libboost-all-dev libgflags-dev libgoogle-glog-dev liblmdb-dev python-pip python-dev python-numpy python-scipy python3-numpy python3-scipy libopencv-dev && apt-get clean && rm -rf /var/lib/apt/lists/* && ln -s /usr/include/x86_64-linux-gnu/gmp.h /usr/include/gmp.h && ln -s /usr/lib/x86_64-linux-gnu/libldap.so /usr/lib/ && ln -s /usr/lib/x86_64-linux-gnu/liblber.so /usr/lib/ && ln -s /usr/lib/libiconv_hook.so.1.0.0 /usr/lib/libiconv.so && ln -s /usr/lib/libiconv_hook.so.1.0.0 /usr/lib/libiconv.so.1

#编译GO
RUN mkdir -pv /opt/soft && \
	cd /opt/soft && \
	echo "编译 GO" && \
	wget -nv https://studygolang.com/dl/golang/go1.10.1.linux-amd64.tar.gz && tar -C /usr/local -xzf go1.10.1.linux-amd64.tar.gz  && mkdir -pv /data/go && go version && go version && \
	cd /opt/soft && \
	echo "编译 Cmake" && \
	wget -nv https://cmake.org/files/v3.11/cmake-3.11.1.tar.gz && tar -zxf cmake-3.11.1.tar.gz && cd cmake-3.11.1 && ./bootstrap && make && make install  && \
	cd /opt/soft && \
	echo "编译 openssl" && \
	wget -nv https://www.openssl.org/source/openssl-1.0.2o.tar.gz  && tar -zxf openssl-1.0.2o.tar.gz  && cd openssl-1.0.2o  &&  ./config shared --prefix=/usr/local/openssl --openssldir=/usr/lib/openssl   && make  && make install  && \
	cd /opt/soft && \
	echo "python 插件" && \
	wget -nv https://bootstrap.pypa.io/get-pip.py && python get-pip.py && python -mpip install -U pyopenssl ndg-httpsclient pyasn1 && python -mpip install --upgrade pip && python get-pip.py && python -mpip install -U numpy && python -mpip install -U matplotlib --ignore-installed six && \
	cd /opt/soft && \
	echo "编译 hiredis" && \
	wget -nv https://github.com/redis/hiredis/archive/v0.13.3.tar.gz && tar -xzf v0.13.3.tar.gz && cd hiredis-0.13.3 && make -j && make install && ldconfig && mkdir -pv /usr/lib/hiredis && cp libhiredis.so /usr/lib/hiredis && mkdir -pv /usr/include/hiredis &&  cp hiredis.h /usr/include/hiredis && \
	cd /opt/soft && \
	echo "编译 inotify-tools" && \
 	wget -nv https://github.com/rvoicilas/inotify-tools/archive/3.20.1.tar.gz && tar -zxf 3.20.1.tar.gz && cd inotify-tools-3.20.1 && ./autogen.sh && ./configure && make && make install && ln -sv /usr/local/lib/libinotify* /usr/lib/ && \
	cd /opt/soft && \
	echo "编译 nghttp2" && \
	wget -nv https://github.com/nghttp2/nghttp2/releases/download/v1.31.1/nghttp2-1.31.1.tar.gz && tar -zxf nghttp2-1.31.1.tar.gz && cd nghttp2-1.31.1 && ./configure && make && make install && \
	cd /opt/soft && \
	echo "编译 jemalloc" && \
...
RUN mkdir -pv /opt/soft && \
	...
	cd /opt/soft && \
	echo "编译 redis 插件" && \
	wget -nv http://pecl.php.net/get/redis-4.0.2.tgz && tar -zxf redis-4.0.2.tgz && cd redis-4.0.2 && /usr/local/php/bin/phpize && ./configure --with-php-config=/usr/local/php/bin/php-config && make && make install && \
	cd /opt/soft && \
	echo "编译 event 插件" && \
	wget -nv http://pecl.php.net/get/event-2.3.0.tgz && tar -zxf event-2.3.0.tgz && cd event-2.3.0 && /usr/local/php/bin/phpize && ./configure   --with-php-config=/usr/local/php/bin/php-config --with-event-core --with-event-extra && make && make install && \
	cd /opt/soft && \
	echo "编译 yaml 插件" && \
	wget -nv http://pecl.php.net/get/yaml-2.0.2.tgz && tar -zxf yaml-2.0.2.tgz && cd yaml-2.0.2 && /usr/local/php/bin/phpize && ./configure  --with-php-config=/usr/local/php/bin/php-config  && make && make install && \
	cd /opt/soft && \
	echo "编译 msgpack 插件" && \
	wget -nv http://pecl.php.net/get/msgpack-2.0.2.tgz && tar -zxf  msgpack-2.0.2.tgz && cd msgpack-2.0.2 && /usr/local/php/bin/phpize && ./configure  --with-php-config=/usr/local/php/bin/php-config  && make && make install && \
	cd /opt/soft && \
	echo "编译 inotify 插件" && \
	wget -nv http://pecl.php.net/get/inotify-2.0.0.tgz  && tar -zxf  inotify-2.0.0.tgz  && cd inotify-2.0.0 && /usr/local/php/bin/phpize && ./configure  --with-php-config=/usr/local/php/bin/php-config  && make && make install && \
	cd /opt/soft && \
	echo "编译 mongodb 插件" && \
	wget -nv http://pecl.php.net/get/mongodb-1.4.3.tgz && tar -zxf mongodb-1.4.3.tgz && cd mongodb-1.4.3 && /usr/local/php/bin/phpize && ./configure  --with-php-config=/usr/local/php/bin/php-config  && make && make install && \
	cd /opt/soft && \
	echo "敏感词过滤PHP 扩展" && \
	wget -nv https://github.com/abulo/php-ext-trie-filter/archive/v1.0.tar.gz && tar zxvf v1.0.tar.gz  && cd php-ext-trie-filter-1.0  &&  /usr/local/php/bin/phpize &&  ./configure   --with-php-config=/usr/local/php/bin/php-config  --with-trie_filter=/usr/local/libdatrie && make && make install && \
	cd /opt/soft && \
	echo "编译PHP-X" && \
	git clone https://github.com/swoole/PHP-X.git && cd PHP-X && cmake . -DPHP_CONFIG_DIR=/usr/local/php/bin && cmake . && make install && \
	cd /opt/soft && \
	echo "编译swoole" && \
	wget -nv https://github.com/swoole/swoole-src/archive/v2.1.3.tar.gz && tar -zxf v2.1.3.tar.gz  && cd swoole-src-2.1.3  &&  /usr/local/php/bin/phpize && ./configure  --enable-swoole-debug --enable-sockets --enable-openssl --with-openssl-dir=/usr/local/openssl --enable-http2 --enable-async-redis --enable-swoole  --enable-coroutine --enable-timewheel --enable-mysqlnd --with-jemalloc-dir=/usr/local/jemalloc --enable-coroutine-postgresql --with-php-config=/usr/local/php/bin/php-config  && make && make install && \
	rm -rf /opt/soft
