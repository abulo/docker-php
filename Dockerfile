...
\
#编译 swoole
&& mkdir -pv /opt/soft && cd /opt/soft && wget -c  https://github.com/swoole/swoole-src/archive/v4.2.13.tar.gz && tar -zxf v4.2.13.tar.gz  && cd swoole-src-4.2.13  &&  /usr/local/php/bin/phpize && ./configure  --enable-openssl  --with-openssl-dir=/usr/local/openssl    --enable-http2   --enable-mysqlnd   --enable-coroutine-postgresql --enable-sockets  --enable-debug-log  --enable-trace-log   --with-php-config=/usr/local/php/bin/php-config && make && make install && rm -rf /opt/soft \
\
#编译swoole_async
