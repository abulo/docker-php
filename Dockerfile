...
#编译swoole_async
RUN mkdir -pv /opt/soft && cd /opt/soft && git clone https://github.com/swoole/async-ext.git --depth=1 && https://github.com/swoole/swoole-src/archive/v4.2.13.tar.gz && tar -zxf v4.2.13.tar.gz &&  cd async-ext  && mv ../swoole-src-4.2.13/thirdparty . &&  /usr/local/php/bin/phpize && ./configure  --with-php-config=/usr/local/php/bin/php-config  && make && make install && rm -rf /opt/soft
