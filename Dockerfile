...
RUN cd /opt/soft && wget -c https://github.com/swoole/php-cp/archive/1.5.0.tar.gz &&  cd php-cp-1.5.0 && /usr/local/php/bin/phpize &&  ./configure  --with-php-config=/usr/local/php/bin/php-config  && make && make install 
...
RUN cp /opt/soft/php-7.1.9/sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm && chmod +x /etc/init.d/php-fpm &&   apt-get clean && rm -rf /opt/soft
