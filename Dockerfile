...
#安装 opencv3
RUN mkdir -pv /opt/soft && wget  https://github.com/opencv/opencv/archive/3.4.1.tar.gz && tar -zxf 3.4.1.tar.gz && mv opencv-3.4.1 opencv && rm 3.4.1.tar.gz && wget  https://github.com/opencv/opencv_contrib/archive/3.4.1.tar.gz && tar -zxf 3.4.1.tar.gz && mv opencv_contrib-3.4.1 opencv_contrib && rm 3.4.1.tar.gz && mkdir -pv /opt/soft/opencv/build && \
...
RUN mkdir -pv /opt/soft && cd /opt/soft && wget https://storage.googleapis.com/golang/go1.10.1.linux-amd64.tar.gz && tar -C /usr/local -xzf go1.10.1.linux-amd64.tar.gz  && ln -sv /usr/local/go/bin/go /bin && mkdir -pv /data/go && go version && rm -rf /opt/soft;
