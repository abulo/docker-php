...
#编译 swoole
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c  https://github.com/swoole/swoole-src/archive/v4.2.12.tar.gz && tar -zxf v4.2.12.tar.gz  && cd swoole-src-4.2.12  &&  /usr/local/php/bin/phpize && ./configure --enable-debug-log --enable-trace-log --enable-sockets --enable-openssl --enable-http2 --enable-swoole  --enable-mysqlnd --enable-coroutine-postgresql --with-openssl-dir=/usr/local/openssl  --with-jemalloc-dir=/usr/local/jemalloc  --enable-debug --with-nghttp2-dir=/usr/local/include/nghttp2  --with-php-config=/usr/local/php/bin/php-config && make && make install && rm -rf /opt/soft
