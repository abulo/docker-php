...
#编译 inotify-tools
RUN mkdir -pv /opt/soft && cd /opt/soft  && wget https://github.com/rvoicilas/inotify-tools/archive/3.20.1.tar.gz  && tar -zxf 3.20.1.tar.gz  && cd inotify-tools-3.20.1  && ./autogen.sh  && ./configure  && make  && make install  && ln -sv /usr/local/lib/libinotify* /usr/lib/  && rm -rf /opt/soft
...
#安装libsodium
RUN mkdir -pv /opt/soft && cd /opt/soft && wget https://github.com/jedisct1/libsodium/archive/1.0.16.tar.gz && tar -zxf 1.0.16.tar.gz && cd libsodium-1.0.16  && ./autogen.sh && ./configure && make && make check && make install  && rm -rf /opt/soft

#python 插件
RUN mkdir -pv /opt/soft && cd /opt/soft; wget https://bootstrap.pypa.io/get-pip.py ;python get-pip.py ;python -mpip install -U numpy ;python -mpip install -U matplotlib --ignore-installed six ;rm -rf /opt/soft;
#安装 opencv3
RUN mkdir -pv /opt/soft wget -c https://github.com/opencv/opencv/archive/3.4.1.tar.gz && tar -zxf 3.4.1.tar.gz && mv opencv-3.4.1 opencv && rm 3.4.1.tar.gz && wget -c https://github.com/opencv/opencv_contrib/archive/3.4.1.tar.gz && tar -zxf 3.4.1.tar.gz && mv opencv_contrib-3.4.1 opencv_contrib && rm 3.4.1.tar.gz && mkdir -pv /opt/soft/opencv/build && \
	cd /opt/soft/opencv/build ; \
		cmake -D CMAKE_BUILD_TYPE=RELEASE \
			-D BUILD_PYTHON_SUPPORT=ON \
			-D CMAKE_INSTALL_PREFIX=/usr/local \
			-D INSTALL_C_EXAMPLES=OFF \
			-D INSTALL_PYTHON_EXAMPLES=ON \
			-D OPENCV_EXTRA_MODULES_PATH=/opt/soft/opencv_contrib/modules \
			-D BUILD_EXAMPLES=ON \
