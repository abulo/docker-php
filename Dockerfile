...
RUN mkdir -pv /opt/soft && cd /opt/soft && wget -c  https://github.com/swoole/swoole-src/archive/v4.0.0.tar.gz && tar -zxf v4.0.0.tar.gz  && cd swoole-src-4.0.0  &&  /usr/local/php/bin/phpize && ./configure   --enable-swoole-debug  --enable-trace-log --enable-sockets --enable-async-redis   --enable-openssl --with-openssl-dir=/usr/local/openssl --enable-http2  --enable-swoole   --enable-coroutine --enable-mysqlnd --with-jemalloc-dir=/usr/local/jemalloc --with-php-config=/usr/local/php/bin/php-config  && make && make install && rm -rf /opt/soft
